...
void DateStruct::readDate(std::string& dateStr) {
    if (dateStr[2] == '.' && dateStr[5] == '.') {
        int d = stoi(dateStr.substr(0, 2)); //стой переводит строку в число
...
}//проверка на разделитель, если что ругается


void DateStruct::printDate(std::ostream& out) {//принимает на вход поток
    out << "Дата: " << dd << '.' << mm << '.' << yy;
}


int DateStruct::getDD() { //получаем информацию с привата, иначе нельзя к нним получать доступ
    return dd;
}
...
void DateStruct::setDD(int value) { //в себя получают инфу и никому не отдают (приравнивают)
    Correct(value, 12, 12);
    dd = value;
}
...
bool DateStruct::isLeapYear(int year) {//проверка на високосный год или отриц год
...
void DateStruct::Correct(int day, int month, int year) {
    ...
    if (month == 2) {
        maxDaysInMonth = isLeapYear(year) ? 29 : 28; //функция високосика здесь нужна
    }
